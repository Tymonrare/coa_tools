...
Важно: если важен pixel perfect, необходимо прописывать `margin=0` т.к. по умолчанию к текстуре добавляется margin=1, и размер ноды будет на 2 пикселя больше по каждой из сторон
